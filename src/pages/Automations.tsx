        ...
        <DialogContent className="sm:max-w-2xl max-h-[90vh] overflow-hidden flex flex-col">
          ...
          <ScrollArea className="flex-1 -mx-6 px-6">
            <div className="space-y-5 pb-4">
              ...
              <div className="space-y-2">
                <div className="flex items-center gap-2">
                  <div className="flex h-6 w-6 items-center justify-center rounded bg-primary text-primary-foreground text-label font-bold">1</div>
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <div className="flex h-6 w-6 items-center justify-center rounded bg-warning text-warning-foreground text-label font-bold">2</div>
                    <Label className="text-xs font-semibold uppercase tracking-wider">Condições (opcional)</Label>
                  </div>
                  <Button variant="ghost" size="sm" className="h-6 text-label" onClick={addCondition}>
                  ...
                  <Card key={i} className="border-warning/20">
                    ...
                    <CardContent className="p-3">
                      <div className="flex items-center gap-2">
                        ...
                        <Input className="h-6 text-label w-24" placeholder="Valor" value={cond.value} onChange={(e) => updateCondition(i, { value: e.target.value })} />
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <div className="flex h-6 w-6 items-center justify-center rounded bg-success text-success-foreground text-label font-bold">3</div>
                    <Label className="text-xs font-semibold uppercase tracking-wider">Ações</Label>
                  </div>
                  <DropdownMenu>
                    <DropdownMenuTrigger asChild>
                      <Button variant="ghost" size="sm" className="h-6 text-label"><Plus className="mr-1 h-3 w-3" />Ação</Button>
                    </DropdownMenuTrigger>
                      ...
                      <CardContent className="p-3">
                        <div className="flex items-center justify-between mb-1.5">
                          <div className="flex items-center gap-1.5">
                            ...
                            <span className="text-label font-semibold">{info.label}</span>
                          </div>
            ...
            <div className="space-y-3 mt-2">
              ...
              {selectedLog.error_message && (
                <div className="space-y-1">
                  ...
                  <pre className="text-label bg-destructive/5 p-2 rounded-md border border-destructive/20 whitespace-pre-wrap">{selectedLog.error_message}</pre>
                </div>
              )}
              {selectedLog.trigger_payload && (
                <div className="space-y-1">
                  <Label className="text-xs">Payload do Trigger:</Label>
                  <pre className="text-label bg-muted p-2 rounded-md overflow-auto max-h-32 whitespace-pre-wrap">{JSON.stringify(selectedLog.trigger_payload, null, 2)}</pre>
                </div>
              )}
              {selectedLog.actions_result && (
                <div className="space-y-1">
                  <Label className="text-xs">Resultado das Ações:</Label>
                  <pre className="text-label bg-muted p-2 rounded-md overflow-auto max-h-32 whitespace-pre-wrap">{JSON.stringify(selectedLog.actions_result, null, 2)}</pre>
                </div>
              )}
            </div>
          )}
