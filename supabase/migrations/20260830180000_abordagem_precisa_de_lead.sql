...
-- PROBLEMA
--
-- Três e-mails de teste apareciam em "Abordagens realizadas". Eram linhas de
-- `emails` com `status = 'sent'` e `contact_id` nulo -- o drill-down as rotulava
-- "Sem lead vinculado" e mesmo assim as somava no card.
--
-- O `contact_id` de um e-mail é resolvido procurando o destinatário entre os
-- contatos. Fica nulo quando quem recebeu não é contato no CRM -- e o CRM não
-- cria contato a partir de destinatário, de propósito.
--
-- POR QUE ISSO ESTAVA ERRADO, E NÃO É SÓ FAXINA
--
-- Uma abordagem sem contato NUNCA pode virar resposta, reunião ou venda: as
-- métricas seguintes todas casam por `contact_id`. Ela entra no denominador da
-- taxa de conversão e não tem como sair do numerador. Quanto mais e-mail para
-- endereço desconhecido, pior a conversão parece -- sem que nada de real tenha
-- piorado.
--
-- E o painel já discordava de si mesmo. `pessoas_abordadas` -- o "· N pessoas"
-- ao lado do número -- JÁ exigia `contact_id IS NOT NULL`, com o comentário:
-- "não dá para afirmar que uma pessoa foi abordada sem saber quem ela é".
-- Então dois números do MESMO card usavam critérios diferentes.
--
-- Efeito esperado: `abordagens` CAI, e passa a contar toques sobre gente
-- conhecida. A diferença para `pessoas_abordadas` continua sendo o DISTINCT --
-- 1 ligação + 1 e-mail para a mesma pessoa seguem valendo 2 abordagens e 1
-- pessoa.
--
-- CUSTO, DECLARADO
--
-- E-mail frio para alguém que ainda não é contato deixa de contar. Foi decisão
-- consciente: o painel mede trabalho sobre leads, e sem lead não há o que medir.
--
-- ESCOPO
--
-- As QUATRO funções mudam juntas -- card, gráfico, lista e desempenho por
-- pessoa. O CLAUDE.md registra por quê: contam a mesma coisa em funções
-- separadas, e mexer numa só faz clicar no número abrir uma lista com outro
-- total.
--
-- Doze sítios ao todo: activities, emails e whatsapp_messages em cada função.
-- Em `sdr_metrics`, o CTE `envio` teve só `enviado` restrito -- `total` e
-- `entregue` são a taxa de entrega da Meta e continuam contando toda tentativa,
-- inclusive para número que não é contato. Restringir os três faria a taxa de
-- entrega esconder recusas em vez de mostrá-las.
--
-- Os corpos vêm COMPLETOS, incluindo a remoção de `meeting` de 20260830150000.
-- É `CREATE OR REPLACE` com corpo inteiro, então aplicar esta migração deixa as
-- funções corretas mesmo que aquela nunca tenha sido aplicada.
--
-- ORDEM IMPORTA, e a ordem padrão já está certa. 20260830150000 vem ANTES pelo
-- nome do arquivo, então aplicar as duas pendentes na ordem natural termina com
-- estas definições. Aplicar SÓ esta à mão, e depois deixar a outra rodar, faria
-- ela sobrescrever com os corpos antigos -- sem `contact_id`. Se for aplicar à
-- mão, aplique as duas, nesta ordem, ou só esta e marque a outra como aplicada.
--
-- O gatilho `promover_lead_para_contatado` NÃO muda: lá a pergunta é "houve
-- contato com esta pessoa?", e ela já pressupõe que há pessoa.
-- ============================================================================
